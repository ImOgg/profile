...
const initAnimations = () => {
    ...
    sections.forEach(section => {
        ...
        gsap.from(section.querySelector('.section-header'), {
            ...
            duration: 1,
            ease: "power3.out",
            immediateRender: !isInitiallyVisible
        });

        if (section.id === 'projects' || section.id === 'skills') {
            // 不使用 stagger 
            if (isInitiallyVisible) {
                ...
                // 正常使用 stagger 
                gsap.from(section.querySelectorAll('.skill-category, .project-card'), {
                    scrollTrigger: {
                        trigger: section,
                        start: "top 70%",
                        end: "bottom 20%",
                    ...
                    ease: "power3.out",
                    onComplete: function () {
                        const cards = section.querySelectorAll(section.id === 'projects' ? '.project-card' : '.skill-category');
                        const groups = {};
            ...
            gsap.from(section.querySelectorAll('.timeline-item, .contact-container > div'), {
                scrollTrigger: {
                    trigger: section,
                    start: "top 70%",
                    end: "bottom 20%",
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    setTimeout(() => {
        ...
        if (visibleSection && visibleSection.id !== 'home') {
            const animationElements = visibleSection.querySelectorAll('.skill-category, .timeline-item, .project-card, .contact-container > div');
            ...
            if (visibleSection.id === 'projects' || visibleSection.id === 'skills') {
                let cardGroups = {};

                if (visibleSection.id === 'projects') {
                    const cards = visibleSection.querySelectorAll('.project-card');
                    cards.forEach(card => {
                        const rect = card.getBoundingClientRect();
                        const row = Math.round(rect.top);
                        if (!cardGroups[row]) cardGroups[row] = [];
                        cardGroups[row].push(card);
                    });
                } else { // skills section
                    const cards = visibleSection.querySelectorAll('.skill-category');
                    cards.forEach(card => {
                        const rect = card.getBoundingClientRect();
                        const row = Math.round(rect.top);
                        if (!cardGroups[row]) cardGroups[row] = [];
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    techStacks.forEach(span => {
        span.addEventListener('mouseenter', () => {
            span.style.backgroundImage = 'var(--gradient)';
        });
